                 ...
                 @if ($posts1->count()>0)
                     @foreach ($posts1 as $post1)
                         <div class="category-content-lead">
                             <a href="{{ route('frontend.details',[$post1->id,$post1->slug])}}" class="post-click" data-id="{{ $post1->id }}">
                                 {{--<p>Clicks: <span class="click-count">{{ $post1->clicks }}</span></p>---}}
                                 <div class="category-content-lead-left">
                                     <img class="img-fluid" src="{{asset('assets/images/post/'.$post1->image_big)}}"
                                          alt="{{ strlen($post1->title)>50 ? mb_substr($post1->title,0,50,'utf-8').'...' :$post1->title}}">
                                 </div>
                   ...
                   @foreach ($posts as $post)
                       <div class="col-md-6 custom-padding">
                           <div class="category-content-single">
                               <a  href="{{ route('frontend.details',[$post->id,$post->slug])}}" class="post-click" data-id="{{ $post->id }}">
                                   <div class="category-content-single-left">
                                       <img class="img-fluid" src="{{asset('assets/images/post/'.$post->image_big)}}"
                                            alt="{{ strlen($post->title)>50 ? mb_substr($post->title,0,50,'utf-8').'...' : $post->title}}">
                                   </div>
                               ...
                               {{--<span id="click-count-{{ $post->id }}">{{ $post->clicks }}</span>--}}
                           </div>
                       </div>
                   @endforeach
               @else
