...
class FrontendController extends Controller
{
    ...
    public function index(){
       ...
       $polls   = PollQuestion::orderBy('id','desc')
        ...
        $hot_news = GeneralSettings::select('hot_news')->first();
        $header_news = DB::table('header_post')
            ->join('posts', 'header_post.category_id', '=', 'posts.category_id')
            ->select('header_post.*','posts.id as post_id','posts.title as post_title','posts.slug','posts.image_big')
            ->orderBy('posts.id','DESC')
            ->limit(12)
            ->where('header_post.status',1)
            ->get();
        $header_post = DB::table('header_post')
    ...
    public function details(Request $request,$id,$slug){
        ...
        $related_posts = Post::orderBy('id','desc')
        ...
        if($data){
            ...
            return view('frontend.details',compact('related_posts','data','ws','sliders','is_recents','slider_rights_firsts','slider_rights_firsts_1'));
        }else{
    ...
    public function incrementClicks(Request $request){
