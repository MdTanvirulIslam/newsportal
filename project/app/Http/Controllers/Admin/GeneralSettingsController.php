...
class GeneralSettingsController extends Controller
{
    ...
    public function update(Request $request){
        ...
        $input  = $request->all();
        $input['hot_news'] = $request->has('hot_news') ? 1 : 1;
    ...
    public function adds(){
        $data = GeneralSettings::find(1);
        return view('admin.generalsettings.adds',compact('data'));
    }
    ...
    public function headerPost(){
        $data = DB::table('header_post')->select('title','description','post_date','bg_color','status','category_id')->first();
        $categories = DB::table('categories')->select('id','title')->get();
        return view('admin.head-post.index',compact('data','categories'));
    }

    public function updateHeaderPost(Request $request){
        $rules = [
